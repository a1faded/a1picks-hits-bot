...
def calculate_pitcher_matchup_grades(df, profile_type):
    """Smooth, probability-based pitcher matchup scoring (no hard thresholds)."""

    for col, default in [
        ('Walk_3Plus_Probability', 15.0),
        ('HR_2Plus_Probability', 12.0),
        ('Hit_8Plus_Probability', 18.0)
    ]:
    ...
    walk_impact = (df['Walk_3Plus_Probability'] - walk_avg) * -0.4

    if profile_type == 'power':
        hr_impact = (df['HR_2Plus_Probability'] - hr_avg) * 0.9
        hit_impact = (df['Hit_8Plus_Probability'] - hit_avg) * 0.2
    else:
        hr_impact = (df['HR_2Plus_Probability'] - hr_avg) * 0.3
        hit_impact = (df['Hit_8Plus_Probability'] - hit_avg) * 0.8

    df['pitcher_matchup_modifier'] = walk_impact + hr_impact + hit_impact

    df['pitcher_matchup_grade'] = pd.cut(
        df['pitcher_matchup_modifier'],
...
def calculate_league_aware_scores(df, profile_type='contact'):
    """Pure probability-weighted scoring with smooth scaling."""
    ...
    hr_factor = df['adj_HR'] / league_hr
    xb_factor = df['adj_XB'] / league_xb
    hit_factor = df['total_hit_prob'] / 35.0
    contact_factor = df['adj_1B'] / league_1b

    k_penalty = (df['adj_K'] - league_k) / league_k
    bb_penalty = (df['adj_BB'] - league_bb) / league_bb
    ...
    score += df['pitcher_matchup_modifier']
